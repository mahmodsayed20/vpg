...
export function Sidebar({ onClose }: { onClose?: () => void }) {
  const { user, activeCategoryId, setActiveCategoryId, openModal, setCategories, items, categories } = useStore()
    ...
    return (
      <div key={node.id}>
        <div
          ...
          style={{ paddingLeft: `${depth * 14 + 8}px`, paddingRight: '6px' }}
        ...
        >
          ...
          {countInNode > 0 && (
            <span className={clsx('text-xs px-1.5 py-0.5 rounded font-mono flex-shrink-0 ml-1',
          ...
          {user?.isAdmin && (
            <div className="hidden group-hover:flex items-center gap-0.5 flex-shrink-0">
              <button title="إضافة فرعي" onClick={e => { e.stopPropagation(); openModal('category', { parentId: node.id }) }}
                className="p-1 rounded hover:bg-bg-border text-text-muted hover:text-text-primary">
                <FolderPlus className="w-3 h-3" />
              </button>
              <button title="تعديل" onClick={e => { e.stopPropagation(); openModal('category', { edit: node }) }}
                className="p-1 rounded hover:bg-bg-border text-text-muted hover:text-text-primary">
                <Edit2 className="w-3 h-3" />
              </button>
              <button title="حذف" onClick={e => { e.stopPropagation(); handleDelete(node.id, node.name) }}
                className="p-1 rounded hover:bg-red-950/40 text-text-muted hover:text-red-400">
  ...
  const flat = search ? categories.filter(c => c.name.toLowerCase().includes(search.toLowerCase())) : []
    ...
    <div className="flex flex-col h-full bg-bg-secondary overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-3 border-b border-bg-border flex-shrink-0">
        ...
        <div className="flex items-center gap-1">
          {user?.isAdmin && (
            <button onClick={() => openModal('category', { parentId: null })}
              className="p-1.5 rounded-lg hover:bg-bg-card text-text-muted hover:text-accent transition-colors" title="قسم جديد">
      ...
      <div className="px-3 py-2 border-b border-bg-border flex-shrink-0">
        <div className="relative">
          ...
          {search && <button onClick={() => setSearch('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-text-muted hover:text-text-primary"><X className="w-3 h-3" /></button>}
        </div>
      </div>
      ...
      <div className="px-3 pt-2 flex-shrink-0">
        <button
          ...
          className={clsx('w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm font-medium transition-all',
            !activeCategoryId ? 'bg-accent/15 text-accent' : 'text-text-secondary hover:bg-bg-card hover:text-text-primary'
          )}
        >
          <FolderOpen className="w-3.5 h-3.5 flex-shrink-0" />
          <span className="flex-1 text-left truncate">كل العناصر</span>
          <span className="text-xs font-mono bg-bg-border text-text-muted px-1.5 py-0.5 rounded flex-shrink-0">{items.length}</span>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden px-3 py-2 space-y-0.5">
        {search ? flat.map(c => (
          <div key={c.id} onClick={() => { setActiveCategoryId(c.id); setSearch(''); onClose?.() }}
            className={clsx('flex items-center gap-2 px-2 py-1.5 rounded-lg cursor-pointer text-sm transition-all',
        ...
        {categories.length === 0 && (
          <div className="text-center py-10 text-text-muted text-xs">
            ...
            <p>لا توجد أقسام</p>
            {user?.isAdmin && <button onClick={() => openModal('category', { parentId: null })} className="mt-2 text-accent hover:underline">إنشاء قسم</button>}
          </div>
        )}
      </div>
    </div>
